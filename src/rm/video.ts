...
export class Video {
    ...
    constructor(id: string | undefined = undefined, title: string | undefined = undefined, channel: string = "Unknown", duration: number = 0) {
        this.id = id;
...
export class ServerVideo extends Video {
    ...
    public set timestamp(ts: number) {
        ...
        if (this.state == 1) {  // If the video is playing
            this._pausedSince = 0;  // This effectively sets the video as playing
        } else if (this.state >= 2) { // If the video is paused
    ...
    playVideo() {
        ...
        debug("[ServerVideo] New duration: " + this._duration);
        ...
        if (this._cbWhenFinished) {
            this._cbWhenFinishedTimeout = setTimeout((id) => {  // , to call the callback
                ...
                if (this.oof2) debug("[ServerVideo] " + oof0 + " OFFFFFFFFFFFFFFFFFFFFFFFFFOOOFFFFFFFFFFFFFFFFFFFFF" + ((new Date().getTime()) - this.oof2));
